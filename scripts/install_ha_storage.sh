...
# Common code - we can seperate into its own script outside of the ha or non-ha scripts
...
if [ "$storage_use_shared_disk" = "true" ]; then
  ...
  storage_use_shared_disk=\"${storage_use_shared_disk}\"
  storage_high_availability=\"${storage_high_availability}\"
  ...
  # For storage_use_shared_disk = true, we assume no local NVMe's (i.e the node is not DenseIO/HPC nodes), since we cannot share nvme across 2 nodes.  hence not checking for nvme disks.
  ...
  if [ "$LOCAL_NODE" = "$NODE1" ]; then
    echo ${hacluster_user_password} | pcs cluster auth --name beegfs_cluster ${NODE1} ${NODE2} -u hacluster
  fi
  ...
  count=1
    ...
    mount -t xfs -o noatime,inode64,nobarrier $device_name /data/ost${ost_count}
    ...
    echo "OSS${id}HA" > /data/ost${ost_count}/beegfs_storage/nodeID
    echo "${id}" > /data/ost${ost_count}/beegfs_storage/nodeNumID
    echo "${id}OST${ost_count}" > /data/ost${ost_count}/beegfs_storage/targetID
    echo "${id}${disk_type}${ost_count}" > /data/ost${ost_count}/beegfs_storage/targetNumID
    ...
    blk_cnt=$(ls /dev/oracleoci/oraclevd* | egrep -v "oraclevda$|oraclevda1$|oraclevda2$|oraclevda3$" | wc -l); echo $blk_cnt
    ct=1;
    while [ $ct -le $blk_cnt ] ; do echo $ct ; mkdir -p /data/ost${ct} ; ct=$((ct+1)) ;  done ;
  fi
  ...
  if [ "$LOCAL_NODE" = "$NODE1" ]; then
    ...
    pcs_group_parameter=" --group ossgroup"

    pcs cluster start --all
    sleep 10s
    pcs status
    pcs cluster cib /root/beegfs_oss_cfg
    pcs -f /root/beegfs_oss_cfg property set stonith-enabled=false
    # In 2 nodes, there is no quorum. hence no-quorum-policy=ignore  instead of no-quorum-policy=stop
    pcs -f /root/beegfs_oss_cfg property set no-quorum-policy=ignore
    pcs -f /root/beegfs_oss_cfg resource defaults resource-stickiness=100
    pcs -f /root/beegfs_oss_cfg resource defaults migration-threshold=1

ost_count=1
for disk in $blk_lst
do
    pcs -f /root/beegfs_oss_cfg resource create ost${ost_count}_fs Filesystem \
    device="${disk}" \
    directory="/data/ost${ost_count}" \
    fstype="xfs" \
    ...
    pcs -f /root/beegfs_oss_cfg resource create beegfs_oss_service systemd:beegfs-storage.service \
    op monitor interval=15s  \
    op start timeout=120 \
    op stop timeout=60 \
    $pcs_group_parameter

    pcs -f /root/beegfs_oss_cfg resource create oss_VIP ocf:heartbeat:IPaddr2 ip=${TARGET_VIP} cidr_netmask=${cidr_netmask} op monitor interval=20s $pcs_group_parameter

    pcs -f /root/beegfs_oss_cfg alert create id=ip_move description="Move IP address using oci-cli" path=/var/lib/pacemaker/ip_move.sh
    pcs -f /root/beegfs_oss_cfg alert recipient add ip_move id=logfile_ip_move value=/var/log/pacemaker_ip_move.log

##pcs -f /root/beegfs_oss_cfg constraint colocation add beegfs_oss_service with ost_fs INFINITY
##pcs -f /root/beegfs_oss_cfg constraint order ost_fs then beegfs_oss_service


ost_count=1
for disk in $blk_lst
do
if [ $ost_count -eq $blk_cnt ]; then
  if [ $blk_cnt -ne 1 ]; then
    # Add for last , last minus 1
    pcs -f /root/beegfs_oss_cfg constraint colocation add ost${blk_cnt}_fs with ost$((blk_cnt-1))_fs INFINITY
    pcs -f /root/beegfs_oss_cfg constraint order ost$((blk_cnt-1))_fs then ost${blk_cnt}_fs
  fi
  pcs -f /root/beegfs_oss_cfg constraint colocation add beegfs_oss_service with ost${ost_count}_fs INFINITY
  pcs -f /root/beegfs_oss_cfg constraint order ost${ost_count}_fs then beegfs_oss_service
elif [ $ost_count -ge 2 ]; then
  pcs -f /root/beegfs_oss_cfg constraint colocation add ost${ost_count}_fs with ost$((ost_count-1))_fs INFINITY
  pcs -f /root/beegfs_oss_cfg constraint order ost$((ost_count-1))_fs then ost${ost_count}_fs
else
  # when ost_count=1 && blk_cnt > 1.  No action needed,  since above logic adds the necessary constraint.
  echo "nothing"
fi
ost_count=$((ost_count+1))
done

    pcs -f /root/beegfs_oss_cfg constraint colocation add oss_VIP with beegfs_oss_service INFINITY
    pcs -f /root/beegfs_oss_cfg constraint order beegfs_oss_service then oss_VIP
  # end of if loop node = node1
  fi

  # Now push the configuration so it becomes active.
  if [ "$LOCAL_NODE" = "$NODE1" ]; then
    pcs cluster cib-push /root/beegfs_oss_cfg
    sleep 5s
  else
