...
if [ "$management_high_availability" = "true" ]; then
  ...
  corosync-cmapctl | grep members
    ...
    pcs_group_parameter=" --group mgsgroup"
      ...
      ##pcs -f /root/beegfs_cfg set no-quorum-policy=stop
      ##pcs -f /root/beegfs_cfg property set no-quorum-policy=ignore
      pcs -f /root/beegfs_cfg property set no-quorum-policy=ignore

      pcs -f /root/beegfs_cfg resource defaults resource-stickiness=100
    ...
    # Add IPaddr2
    pcs -f /root/beegfs_cfg resource create mgs_VIP ocf:heartbeat:IPaddr2 ip=${TARGET_VIP} cidr_netmask=${cidr_netmask} op monitor interval=20s  $pcs_group_parameter
    ...
    if [ "$LOCAL_NODE" = "$NODE1" ]; then
      ...
      mount -o defaults $device_name /data/mgt1
